...
#define getSockaddrIn(addr) ((struct sockaddr_in *)(void*)(addr))
#define getSinAddr(addr) (getSockaddrIn(addr)->sin_addr)
...
/**
 * Set socket to listen on given multicast address. Should be invoked only
 * on a socket also bound to the multicast address, in order not to accidentally
 * get traffic sent to another multicast address with same port.
 * For SSM networks (232.0.0.0/8), pass source of server as well, if available.
 */
static int mcastListen(int sock, net_if_t *net_if, struct sockaddr_in *addr,
		       struct in_addr *src) {
#ifdef IP_ADD_SOURCE_MEMBERSHIP
    if((addr->sin_addr.s_addr & htonl(0xff000000)) == htonl(0xe8000000) &&
       src != NULL) {
	int r;
	struct ip_mreq_source ms;
	ms.imr_multiaddr = addr->sin_addr;
	ms.imr_interface = net_if->addr;
	ms.imr_sourceaddr = *src;
	r=setsockopt(sock, SOL_IP, IP_ADD_SOURCE_MEMBERSHIP,
			  (char*)&ms, sizeof(ms));
	if(r < 0) {
	    perror("Subscribe to multicast group");
	    exit(1);
	}
	return 0;
    }
...
net_if_t *getNetIf(const char *wanted) {
	...
	ifend = ((uintptr_t)ifc.ifc_req) + (uintptr_t) ifc.ifc_len;
	chosen=NULL;

	for (ifrp = ifc.ifc_req; (uintptr_t)ifrp < ifend; ifrp++) {
	...
	if(!chosen) {
		...
		fprintf(stderr, "\t%s\t%s\n",
			ifrp->ifr_name,
			udpc_getIpString(getSockaddrIn(&ifrp->ifr_addr), buffer)
			);
	    }
	    exit(1);
	}
...
int makeSocket(addr_type_t addr_type,
	...
	mcastListen(lastSocket, net_if, tmpl, src);
	return -1;
    ...
    if(addr_type == ADDR_TYPE_MCAST)
	mcastListen(s, net_if, &myaddr, src);
#ifdef WINDOWS
    lastSocket=s;
#endif
    return s;
...
void getDefaultMcastAddress(net_if_t *net_if, struct sockaddr_in *mcast) {
    ...
    mcast->sin_addr.s_addr &= htonl(0x0001ffff);
    mcast->sin_addr.s_addr |= htonl(0xe9fe0000);
